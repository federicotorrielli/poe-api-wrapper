...
class AsyncPoeApi:
    ...
    async def send_request(
        ...
        try:
            ...
            if status_code == 403:
                if ratelimit < 2:
                    logger.warning(
                        f"Received 403 status code, retrying... (attempt {ratelimit + 1}/2)"
                    )
                    ...
                    raise Exception(
                        "Max retries reached after receiving 403 status code"
                    )
        ...
        if chatId == None and chatCode == None:
            try:
                ...
                if message_data["data"] == None and message_data["errors"]:
                    ...
                    if not chat_data:
                        raise RuntimeError(
                            f"Failed to create chat: {message_data['data']['messageEdgeCreate'].get('statusMessage', 'Unknown error')}"
                        )
    ...
    async def get_previous_messages(
        ...
        try:
            ...
            raise RuntimeError(
                "Thread not found. Make sure the thread exists before getting messages."
            )
    ...
    async def create_bot(
        ...
        if botCategory:
            ...
            botCategory = (
                ...
                if base_model in botCategories["video"]
                else None
            )
    ...
    async def share_chat(
        ...
        if response_json["data"]["messagesShare"]:
            ...
            logger.error("An error occurred while sharing the messages")
            return None
    ...
    async def import_chat(self, bot: str = "", shareCode: str = ""):
        ...
        if response_json["data"]["continueChatFromPoeShare"]["status"] == "success":
            logger.info("Chat imported successfully")
            ...
            logger.error("An error occurred while importing the chat")
            return None
    ...
    async def create_group(self, group_name: str = None, bots: list = []):
        ...
        if bots == []:
            raise ValueError("Please provide at least one bot to create a group.")
    ...
    async def set_context_optimization(self, chat_id: int, enabled: bool) -> dict:
        variables = {"chatId": chat_id, "isContextOptimizationOn": enabled}
